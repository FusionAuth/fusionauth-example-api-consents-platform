...
app.get('/login', (req, res, next) => {
  ...
  res.redirect(302, `${fusionAuthURL}/oauth2/authorize?client_id=${clientId}&response_type=code&redirect_uri=http://localhost:${port}/oauth-redirect&state=${userSessionCookie?.stateValue}&code_challenge=${userSessionCookie?.challenge}&code_challenge_method=S256&scope=profile%20email%20openid%20offline_access`)
});
//end::login[]
...
app.get('/authorize', (req, res, next) => {
  ...
  res.redirect(302, `${fusionAuthURL}/oauth2/authorize?client_id=${clientId}&response_type=code&redirect_uri=http://localhost:${port}/oauth-redirect&state=${userSessionCookie?.stateValue}&code_challenge=${userSessionCookie?.challenge}&code_challenge_method=S256&scope=profile%20email%20openid%20offline_access%20accounts.read%20creditScore.read%20investments.read%20profile.read%20transactions.read`)
...
app.get('/oauth-redirect', async (req, res, next) => {
  ...
  try {
    ...
    res.cookie(userToken, accessToken.access_token, { httpOnly: true })
    res.cookie(refreshToken, accessToken.refresh_token, { httpOnly: true })

    res.cookie(userDetails, accessToken.id_token);
